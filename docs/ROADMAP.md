...
- [x] **删 legacyParse**(`src/utils/import.ts:97`):300+ 行废弃 fallback,学习流稳定后移除
- [x] **硬去重快速路径**:导入前按 `amount+date+note` hash 预筛,精确重复不走 O(n²) 模糊路径
- [x] **导出修复**(`src/utils/export.ts`):CSV 加 BOM + 引号转义;JSON 加 `schemaVersion`
- [x] **PWA 补图标**:manifest 已引用 192/512 PNG,`public/` 下只有 svg,生成位图
- [x] **统一 CSV 解析**:`analyzer` / `universalParser` / `import` 三份手写合并为一份,处理 BOM 与多行引号字段
- [x] **静默错误可观测**:`catch { /* noop */ }` 换成结构化日志 / Toast

## P1 · 架构升级(中期,~1–2 月)

目标:打地基,把「改一处痛四处」变成「注册即扩展」。

- [x] **LLMClient 统一抽象**:收敛 4 份 fetch(`src/llm/service.ts` 三函数 + `src/bill-analyzer/aiMapper.ts`);Provider 适配器(OpenAI 兼容为基,预留 Anthropic/Google);抽 mock 边界让 LLM 可测
- [x] **Task 注册抽象**:`{ prompt, parse, validate, fallback }` 注册式,新 AI 任务零改调度
