...
- [x] **R1 视觉与交互焕新(v1.6.0,2026-09-15,PR #27)** —— 现代亲和风:token 重塑(圆角 2px→12px 档/柔和阴影/字阶);lucide SVG 图标系统替换 emoji(分类/导航/AI 任务);首页/统计/明细/预算核心页精修;Dialog 键盘交互补课(Esc/焦点圈定/焦点归还)
- [x] **R2 体验功能升级(v1.7.0,2026-09-15,PR #28)** —— 流式输出(SSE+自动降级,体验队列队首);明细页规模化(DB v14 note 索引下推+虚拟滚动+日期/金额筛选,修复筛选态截断);统计深度(月环比/同比柱状图+完整排行);设置页信息架构重组(四组 tab+危险区隔离);预算建议与双档预警
