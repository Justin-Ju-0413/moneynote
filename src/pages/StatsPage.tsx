      ...
      <div className="px-5 space-y-5 md:px-8 md:space-y-6 lg:px-10 lg:space-y-8">
        ...
        <Card>
          <div className="grid grid-cols-3 gap-2 text-center">
            <div>
              <p className="text-[11px] text-text-muted mb-2">支出</p>
              <p className="text-xl md:text-2xl lg:text-3xl font-heading tabular-nums text-expense">¥{stats.totalExpense.toFixed(2)}</p>
            </div>
            <div>
              <p className="text-[11px] text-text-muted mb-2">收入</p>
              <p className="text-xl md:text-2xl lg:text-3xl font-heading tabular-nums text-income">¥{stats.totalIncome.toFixed(2)}</p>
            </div>
            <div>
              <p className="text-[11px] text-text-muted mb-2">结余</p>
              <p className="text-xl md:text-2xl lg:text-3xl font-heading tabular-nums text-text">¥{stats.netIncome.toFixed(2)}</p>
            </div>
          </div>
