...
2. category 必须是以下之一: food, transport, shopping, entertainment, housing, medical, education, other
...
function validateAndNormalize(parsed: Record<string, unknown>): LLMParseResult | null {
  ...
  // category
  let category = 'other'
  if (typeof parsed.category === 'string' && VALID_CATEGORIES.includes(parsed.category as typeof VALID_CATEGORIES[number])) {
...
2. 每个元素的 category 必须是: food, transport, shopping, entertainment, housing, medical, education, other
...
export function parseBatchResponse(raw: string, expectedCount: number): (BatchClassifyItem | null)[] {
  ...
  return parsed.slice(0, expectedCount).map((item: unknown) => {
    ...
    const category = typeof obj.category === 'string' && VALID_CATEGORIES.includes(obj.category as typeof VALID_CATEGORIES[number])
      ? obj.category
      : 'other'
