...
export function buildAuditSystemPrompt(task: AuditTask): string {
  const base = `你是个人账单审计助手。只返回严格 JSON 对象，格式为 {"suggestions":[{"type":"category","transactionIds":[123],"result":"food","confidence":0.9,"reason":"依据商户和备注"}]}。suggestions 只能是对象数组，不能是字符串数组。type 只能为 category、duplicate、anomaly、summary。category 建议的 result 只能为：${CATEGORY_LIST}。必须使用输入里的 id（数字），不要编造不存在的交易。`
