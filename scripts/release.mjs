 ...
 * 流程：
 *   1. 校验 git 干净 + 位于 main
 *   2. 计算新版本（SemVer 递增，或直接给 <x.y.z>）
 *   3. 同步版本号：package.json + src/utils/constants.ts（APP_VERSION 单一来源）
 *   4. 校验 CHANGELOG.md 已含 `## [v] - YYYY-MM-DD` 段（缺失则中止并提示）
 *   5. 自动 commit（chore: version bump 1.x.y→1.x.z）
 *   6. git tag -a vX.Y.Z + push main + push tag
 *   7. 从 CHANGELOG 段提取 notes，调 gh release create（title 统一 = tag 名）
 *
 * 前置：gh CLI 已认证（gh auth status）；CHANGELOG 版本段已由开发流程写好并合入 main。
...
const target = nextVersion(current, arg ?? 'patch')
console.log(`发布 v${current} → v${target}${dryRun ? '（dry-run）' : ''}`)

// ── 3. 同步版本号（package.json + APP_VERSION）──
...
// ── 5–7. 写文件 → commit → tag → push → release ──
